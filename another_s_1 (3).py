        ...
        with (st.spinner("Выполняется расчет...")):
            ...
            if not df.empty:
                ...
                with tab1:
                    ...
                    def update_table(df_in):
                        ...
                        with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
                            filtered_df.to_excel(writer, index=False, sheet_name="Детальная таблица по органам")
                        st.download_button("Скачать таблицу", data=buffer.getvalue(),
                                           file_name="radiation_results.xlsx",
                ...
                with tab2:
                    ...
                    buffer = io.BytesIO()
                    with pd.ExcelWriter(buffer, engine="xlsxwriter") as writer:
                        summary.to_excel(writer, index=False, sheet_name="Сводная таблица по пациентам")
                    st.download_button("Скачать сводку",
                                       data=buffer.getvalue(),
                                       file_name="radiation_summary.xlsx",
                                       mime="application/vnd.ms-excel")
